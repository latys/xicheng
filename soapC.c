...
SOAP_SOURCE_STAMP("@(#) soapC.c ver 2.8.37 2016-11-03 12:26:44 GMT")
...
SOAP_FMAC3 struct add * SOAP_FMAC4 soap_in_add(struct soap *soap, const char *tag, struct add *a, const char *type)
{
	...
	{	a = (struct add *)soap_id_forward(soap, soap->href, (void*)a, 0, SOAP_TYPE_add, SOAP_TYPE_add, sizeof(struct add), 0, NULL, NULL);
		if (soap->body && soap_element_end_in(soap, tag))
			return NULL;
	}
	return a;
...
{
	struct add *p;
	struct add *a = (struct add*)soap_malloc((soap), (n = (n < 0 ? 1 : n)) * sizeof(struct add));
	for (p = a; p && n--; p++)
...
SOAP_FMAC3 struct addResponse * SOAP_FMAC4 soap_in_addResponse(struct soap *soap, const char *tag, struct addResponse *a, const char *type)
{
	...
	{	a = (struct addResponse *)soap_id_forward(soap, soap->href, (void*)a, 0, SOAP_TYPE_addResponse, SOAP_TYPE_addResponse, sizeof(struct addResponse), 0, NULL, NULL);
		if (soap->body && soap_element_end_in(soap, tag))
			return NULL;
	}
	return a;
...
SOAP_FMAC3 struct addResponse * SOAP_FMAC4 soap_new_addResponse(struct soap *soap, int n)
{
	...
	struct addResponse *a = (struct addResponse*)soap_malloc((soap), (n = (n < 0 ? 1 : n)) * sizeof(struct addResponse));
	for (p = a; p && n--; p++)
